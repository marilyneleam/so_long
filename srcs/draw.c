...
/*   Updated: 2022/03/22 22:38:17 by mleam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_elements(t_all *all, int i, int j)
{
	if (all->map.tab[i][j] == '1')
		mlx_put_image_to_window(all->data.mlx, all->data.mlx_win,
			all->text.wall.img, j * 32, i * 32);
	if (all->map.tab[i][j] == '0')
		mlx_put_image_to_window(all->data.mlx, all->data.mlx_win,
			all->text.ground.img, j * 32, i * 32);
	if (all->map.tab[i][j] == 'E')
		mlx_put_image_to_window(all->data.mlx, all->data.mlx_win,
			all->text.exit.img, j * 32, i * 32);
}
...
void	draw_collectibles(t_all *all, int i, int j)
{
	...
	if (all->map.tab[i][j] == 'C')
		mlx_put_image_to_window(all->data.mlx, all->data.mlx_win,
			all->text.collectibles.img, j * 32, i * 32);
}
...
void	draw_map(t_all *all)
{
	...
	mlx_put_image_to_window(all->data.mlx, all->data.mlx_win,
		all->text.player.img, all->map.pos_x * 32,
		all->map.pos_y * 32);
}
